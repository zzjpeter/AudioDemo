...
// 这里的InputBus和OutputBus 是针对RemoteI/O而言 （可以简单理解为MixUnit是嵌入到 RemoteI/O Unit的 从设备(麦克风)(输入单元Element0 和 输出到设备(扬声器)单元Element1 之间的)）
//上面是参照AUGraph结合RemoteI/O Unit与Mixer Unit的音频流向图得出的
//AudioUnitElement
...
@implementation AudioCommonManager
...
- (void)setupAudioUnitBase{
    ...
    if (self.audioCommonManagerType == AudioCommonManagerAUGraphType) {
        [self setupMixUnit];//设置mixerUnit（音频混合Unit）
    }
    ...
    if (self.audioCommonManagerType == AudioCommonManagerAUGraphType) {
        ...
        checkStatus(status, "init augraph fail");
        return;
    }
...
- (void)initAudioUnit
{
    if (self.audioCommonManagerType == AudioCommonManagerAUGraphType) {
        ...
        //AUGraph的方式进行AudioUnit的初始化
           //新建AUGraph，并添加两个AUNode，一个是RemoteI/O Unit的节点，一个是Mixer Unit的节点
        
        // augraph
        checkStatus(NewAUGraph(&auGraph), "NewAUGraph error");
        checkStatus(AUGraphOpen(auGraph), "open graph fail");
        ...
        AUNode outputNode;
        checkStatus(AUGraphAddNode(auGraph, &desc, &outputNode), "add node fail");
        checkStatus(AUGraphNodeInfo(auGraph, outputNode, NULL, &_audioUnit), "get audio unit fail");
        ...
        AUNode mixNode;
        checkStatus(AUGraphAddNode(auGraph, &mixAudioDesc, &mixNode), "add node fail");
        checkStatus(AUGraphNodeInfo(auGraph, mixNode, NULL, &_mixUnit), "get audio unit fail");
        
        // connect
        checkStatus(AUGraphConnectNodeInput(auGraph, mixNode, MIX_UNIT_OUTPUT_BUS, outputNode, kOutputBus), "connect fail"); //从字面看是把mixNode的输出作为outputNode的输入。但是在bus的参数设置上，为什么Remote I/O Unit的bus不是inputBus？因为Remote I/O Unit的输入域有两个Bus，inputBus对应的是麦克风的输入，outputBus对应的是app发送给Remote I/O Unit的数据。这里Mixer Unit是把人声和伴奏混合后，输出给Remote I/O Unit，相当于app发送数据给Remote I/O Unit，所以这里应该填outputBus。【详解参照顶上】
...
- (void)initOrGetAudioInputFormat
{
    ...
    if (self.audioCommonManagerType == AudioCommonManagerExtAudioFileRefType) {
...
//7.2
- (void)initBuffer {
    buffer = malloc(CONST_BUFFER_SIZES);
}
...
#pragma mark setupMixUnit
- (void)setupMixUnit {
    // setup mix unit
    UInt32 busCount = 2;
    UInt32 size = sizeof(UInt32);
    checkStatus(AudioUnitSetProperty(self.mixUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &busCount, size), "set property fail");
    
    checkStatus(AudioUnitGetProperty(self.mixUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &busCount, &size), "get property fail");
    checkStatus(AudioUnitGetProperty(self.audioUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Global, kInputBus, &busCount, &size), "get property fail");
    
    AURenderCallbackStruct callback0;
    callback0.inputProc = mixCallback0;
    callback0.inputProcRefCon = (__bridge void *)self;
    checkStatus(AudioUnitSetProperty(self.mixUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &callback0, sizeof(AURenderCallbackStruct)), "add mix callback fail");
    checkStatus(AudioUnitSetProperty(self.mixUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &audioOutputFormat, sizeof(AudioStreamBasicDescription)), "set mix format fail");
    
    
    AURenderCallbackStruct callback1;
    callback1.inputProc = mixCallback1;
    callback1.inputProcRefCon = (__bridge void *)self;
    checkStatus(AudioUnitSetProperty(self.mixUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS1, &callback1, sizeof(AURenderCallbackStruct)), "add mix callback fail");
    checkStatus(AudioUnitSetProperty(self.mixUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS1, &audioOutputFormat, sizeof(AudioStreamBasicDescription)), "set mix format fail");
}
...
- (void)start {
    ...
    if (self.audioCommonManagerType == AudioCommonManagerAUGraphType) {
        OSStatus status = AUGraphStart(auGraph);
        checkStatus(status, "AUGraphStart");
        return;
    }
...
- (void)stop {
    ...
    if (self.audioCommonManagerType == AudioCommonManagerAUGraphType) {
        OSStatus status = AUGraphStop(auGraph);
        checkStatus(status, "stop graph fail");
...
- (void)finished {
    ...
    if (self.audioCommonManagerType == AudioCommonManagerAUGraphType) {
        checkStatus(AUGraphUninitialize(auGraph), "uninit graph fail");
        checkStatus(DisposeAUGraph(auGraph), "dispose fail");
        auGraph = NULL;
        self.audioUnit = nil;
        self.mixUnit = nil;
    }
...
-(NSString *)fileName:(NSInteger)type {
    NSMutableString *strM = [NSMutableString stringWithFormat:@"%@",NSStringFromClass(AudioCommonManager.class)];
    switch (type) {
        case 1:
        {
...
static OSStatus recordingCallback(void *inRefCon,
    ...
    AudioCommonManager *self = (__bridge AudioCommonManager*) inRefCon;
...
{
    AudioCommonManager *self = (__bridge AudioCommonManager *)(inUserData);
...
static OSStatus mixCallback0(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    NSInteger bytes = CONST_BUFFER_SIZES < ioData->mBuffers[0].mDataByteSize * 2 ? CONST_BUFFER_SIZES : ioData->mBuffers[0].mDataByteSize * 2;
    
    bytes = [self->inputSteam read:self->buffer maxLength:bytes];
    
    for (int i = 0; i < bytes; ++i) {
        ((Byte*)ioData->mBuffers[0].mData)[i/2] = self->buffer[i];
    }
    ioData->mBuffers[1].mDataByteSize = (UInt32)bytes / 2;
    
    NSLog(@"mixCallback0:size0:%ld",(long)ioData->mBuffers[0].mDataByteSize);
    NSLog(@"mixCallback0:size1:%ld",(long)ioData->mBuffers[1].mDataByteSize);
...
static OSStatus mixCallback1(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    AudioCommonManager *self = (__bridge AudioCommonManager *)inRefCon;
    memcpy(ioData->mBuffers[0].mData, self->_bufferList->mBuffers[0].mData, self->_bufferList->mBuffers[0].mDataByteSize);
    ioData->mBuffers[0].mDataByteSize = self->_bufferList->mBuffers[0].mDataByteSize;
    
    NSLog(@"mixCallback1:size:%ld",(long)ioData->mBuffers[0].mDataByteSize);
