...
static const uint32_t CONST_BUFFER_SIZES = 2048*2*10;//2048*2*10;//0x10000

@interface AudioAUGraphManager ()
{
    OSStatus status;
...
@implementation AudioAUGraphManager
...
- (void)setupAudioUnitBase{
    ...
    //8.通过设置输入输出格式创建音频转码器
    AudioStreamBasicDescription audioInputFormat = {0};
    ...
    checkStatus(AUGraphInitialize(auGraph), "init augraph fail");
...
- (void)setupMixUnit {
    ...
    checkStatus(AudioUnitSetProperty(_mixUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &busCount, size), "set property fail");
    
    checkStatus(AudioUnitGetProperty(_mixUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &busCount, &size), "get property fail");
    checkStatus(AudioUnitGetProperty(_audioUnit, kAudioUnitProperty_ElementCount, kAudioUnitScope_Global, kInputBus, &busCount, &size), "get property fail");
    
    AURenderCallbackStruct callback0;
    callback0.inputProc = mixCallback0;
    callback0.inputProcRefCon = (__bridge void *)self;
    checkStatus(AudioUnitSetProperty(_mixUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &callback0, sizeof(AURenderCallbackStruct)), "add mix callback fail");
    checkStatus(AudioUnitSetProperty(_mixUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS0, &audioOutputFormat, sizeof(AudioStreamBasicDescription)), "set mix format fail");
    
    
    AURenderCallbackStruct callback1;
    callback1.inputProc = mixCallback1;
    callback1.inputProcRefCon = (__bridge void *)self;
    checkStatus(AudioUnitSetProperty(_mixUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS1, &callback1, sizeof(AURenderCallbackStruct)), "add mix callback fail");
    checkStatus(AudioUnitSetProperty(_mixUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, MIX_UNIT_INPUT_BUS1, &audioOutputFormat, sizeof(AudioStreamBasicDescription)), "set mix format fail");
}
...
- (void)stop {
    ...
    checkStatus(AUGraphStop(auGraph), "stop graph fail");
...
static OSStatus recordingCallback(void *inRefCon,
    ...
    AudioAUGraphManager *self = (__bridge AudioAUGraphManager*) inRefCon;
