...
#import "StreamClientManager.h"
#import <netdb.h>
#include <pthread.h>

#define Host_Addr ("192.168.1.102")

#define PRINTERROR(LABEL)    printf("%s err %4.4s %ld\n", LABEL, (char *)&err, err)

static const int port = 51515;                      // socket端口号
static const unsigned int kNumAQBufs = 3;           // audio queue buffers 数量
static const size_t kAQBufSize = 128 * 1024;        // buffer 的大小 单位是字节
static const size_t kAQMaxPacketDescs = 512;        // ASPD的最大数量
...
@interface StreamClientManager ()<AVCaptureVideoDataOutputSampleBufferDelegate>

@end

@implementation StreamClientManager
